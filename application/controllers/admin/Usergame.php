...
Class Usergame extends MY_Controller
{
    ...
    function indexajax()
    {
        ...
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=104&un=' . $username . '&nn=' . $nickname . '&m=' . $phone . '&fd=' . $fieldname . '&srt=' . $timkiemtheo . '&ts=' . urlencode($toDate) . '&te=' . urlencode($fromDate) . '&dl=' . $typetaikhoan . '&p=' . $pages . '&tr=' . $record . '&bt=' . $taikhoanbot.'&email='.$email.'&lk='.$typetk);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
    ...
    function uservinplayajax()
    {
        ...
        $typetk = $this->input->post("typetk");
        $email = $this->input->post("email");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=104&un=' . $username . '&nn=' . $nickname . '&m=' . $phone . '&fd=' . $fieldname . '&srt=' . $timkiemtheo . '&ts=' . urlencode($toDate) . '&te=' . urlencode($fromDate) . '&dl=' . $typetaikhoan . '&p=' . $pages . '&tr=' . $record . '&bt=0'.'&email='.$email.'&lk='.$typetk);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function userbotajax()
    {
        ...
        $pages = $this->input->post("pages");
        $record = $this->input->post("record");
        $datainfo = $this->get_data_curl($this->config->item('api_backend') . '?c=104&un=' . $username . '&nn=' . $nickname . '&m=' . $phone . '&fd=' . $fieldname . '&srt=' . $timkiemtheo . '&ts=' . urlencode($toDate) . '&te=' . urlencode($fromDate) . '&dl=' . $typetaikhoan . '&p=' . $pages . '&tr=' . $record . '&bt=1'.'&email=&lk=0');
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
