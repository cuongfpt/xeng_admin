...
Class Transaction extends MY_Controller
{
    ...
    function indexajax()
    {
      ...
      //  var_dump($this->config->item('api_backend2') . '?c=3&nn=' . $nickname . '&un=' . "" . '&ts=' . urlencode($toDate) . '&te=' . urlencode($fromDate) . '&mt=' . $money . '&ag=' . $servicename . '&sn=' . "" . '&p=' . $pages . '&lk=1&tr'.$record);
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
    ...
    function usergiftcodeajax()
    {
        ...
        $money = $this->input->post("money");
        $pages = $this->input->post("pages");
        $datainfo = $this->get_data_curl($this->config->item('api_backend2') . '?c=3&nn=' . $nickname . '&un=' . "" . '&ts=' . urlencode($toDate) . '&te=' . urlencode($fromDate) . '&mt=' . $money . '&ag=GiftCode&sn=' . "" . '&p=' . $pages . '&lk=1');
        if (isset($datainfo)) {
            echo $datainfo;
        } else {
            echo "Bạn không được hack";
    ...
    function  refundi2bajax()
    {
        ...
        $datainfo = $this->get_data_curl('http://api.vinplay.com:9081/i2b?error_code=00&token=' . $token);
        if (isset($datainfo)) {
            $data = array(
